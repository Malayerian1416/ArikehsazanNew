...
class LeaveFlow extends Model
{
    ...
    public static function automate(): array
    {
        ...
        switch ($previous_role_priority){
            case 1:{
                $previous_role_id == 0 || $max_priority == 1 ? $current_role_id = self::query()->where("priority", "=", $previous_role_priority)->first()->role_id :
        ...
        $current = self::query()->where("role_id","=",$previous_role_id)->first();
        return [
            "previous_role_id"=>$previous_role_id,
            "current_role_id"=>$current_role_id,
            "next_role_id"=>$next_role_id,
            "is_read"=> $next_role_id == 0 ? 1 : 0,
            "is_finished" => $current != null && $current->is_main == 1 ? 1 : $is_finished
        ];
    }
    public static function refer($id)
    {
        $leave_automation = LeaveAutomation::query()->with(["automationable.staff.role"])->findOrFail($id);
        ...
        switch ($current_role_priority){
            case ($current_role_priority == $min_priority):{
                $previous_role_id = self::query()->where("priority","=",$current_role_priority)->first()->role_id;
                $current_role_id = $leave_automation->automationable->staff->role->id;
                break;
            }
