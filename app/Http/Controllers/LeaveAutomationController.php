...
class LeaveAutomationController extends Controller
{
    public function get_automation_items(){
        ...
        try {
            $leave_automations_inbox = LeaveAutomation::query()
                ->with(["automationable.staff","automationable.user"])->where("is_finished","=",0)->where("current_role_id","=",Auth::user()->role->id)->get();
    ...
    public function automate_sending(Request $request,$id): \Illuminate\Http\RedirectResponse
    {
        ...
        try {
            ...
            $automation->signs()->updateOrCreate(["leave_automation_id " => $automation->id,"user_id" => Auth::id(),],["sign" => Auth::user()->sign]);
            DB::commit();
    ...
    public function refer($id): \Illuminate\Http\RedirectResponse
    {
        ...
        try {
            DB::beginTransaction();
            $automation = LeaveAutomation::query()->findOrFail($id);
            LeaveFlow::refer($automation->id);
            $automation->automationable->update(["is_approved" => 0,"current_status" => 1]);
            $automation->signs()->updateOrCreate(["leave_automation_id" => $automation->id,"user_id" => Auth::id(),],["sign" => Auth::user()->sign]);
            DB::commit();
    ...
    public function approve($id): \Illuminate\Http\RedirectResponse
    {
        ...
        try {
            ...
            $automation = LeaveAutomation::query()->findOrFail($id);
            $automation->update(LeaveFlow::automate());
            $automation->automationable->update(["is_approved" => 1,"current_status" => 0]);
            $automation->signs()->updateOrCreate(["leave_automation_id " => $automation->id,"user_id" => Auth::id(),],["sign" => Auth::user()->sign]);
            DB::commit();
            $message = '';
            ...
            DB::rollBack();
            return redirect()->back()->with(["action_error" => $ex->getMessage().$ex->getLine().$ex->getFile()]);
        }
    }
    ...
    public function destroy($id): \Illuminate\Http\RedirectResponse
    {
        try {
            DB::beginTransaction();
            $automation = LeaveAutomation::query()->findOrFail($id);
            $automation->automationable()->delete();
            $automation->delete();
            DB::commit();
            return redirect()->route("LeaveAutomation.automation")->with(["result" => "deleted"]);
        }
        catch (Throwable $ex){
            DB::rollBack();
            return redirect()->back()->with(["action_error" => $ex->getMessage()]);
